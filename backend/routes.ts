...
Format responses with clear structure using markdown when helpful.`;
  ...
  app.post("/api/simulator/attack", async (req, res) => {
    try {
      ...
      console.error("Simulator API Error:", error);
      res.status(503).json({ error: "Failed to inject attack", details: String(error) });
    }
  });

  app.get("/api/simulator/detections", async (req, res) => {
    try {
  ...
  app.post("/api/simulator/protection", async (req, res) => {
    try {
      ...
      console.error("Simulator API Error:", error);
      res.status(503).json({ error: "Failed to execute protection action", details: String(error) });
    }
  });
