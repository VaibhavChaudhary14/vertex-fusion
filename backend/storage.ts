import {
  ...
  mitigationActions, mitigationLogs,
  type InsertMitigationLog, type MitigationLog
} from "@shared/schema";
import { db } from "./db";
...
export interface IStorage {
  ...
  createMitigationLog(log: InsertMitigationLog): Promise<MitigationLog>;
  getMitigationLogs(limit?: number): Promise<MitigationLog[]>;
}
...
class MemoryStorage implements IStorage {
  ...
  private mitigationLogs: MitigationLog[] = [];
  ...
  async upsertUser(userData: UpsertUser): Promise<User> {
    const id = userData.id || Math.random().toString();
    const user: User = {
      ...userData,
      id,
      createdAt: new Date(),
  ...
  async getAlerts(userId?: string, limit: number = 50): Promise<Alert[]> {
    ...
    return alerts.sort((a, b) => (b.createdAt?.getTime() ?? 0) - (a.createdAt?.getTime() ?? 0)).slice(0, limit);
  }
  ...
  async createAlert(alert: InsertAlert): Promise<Alert> {
    ...
    // Trigger automated mitigation if applicable
    const { MitigationEngine } = await import("./mitigation");
    MitigationEngine.evaluateAlert(newAlert);
  ...
  async getAlertsLast24h(): Promise<number> {
    ...
    return this.alerts.filter(a => (a.createdAt?.getTime() ?? 0) > yesterday.getTime()).length;
  }
  ...
  async createMitigationLog(log: InsertMitigationLog): Promise<MitigationLog> {
    const newLog: MitigationLog = {
      ...log,
      id: Math.random().toString(),
      actionId: log.actionId ?? null,
      message: log.message ?? null,
      alertId: log.alertId ?? null,
      executedAt: new Date(),
    };
    this.mitigationLogs.push(newLog);
    return newLog;
  }

  async getMitigationLogs(limit: number = 20): Promise<MitigationLog[]> {
    return this.mitigationLogs
...
export class DatabaseStorage implements IStorage {
  ...
  async createMitigationLog(log: InsertMitigationLog): Promise<MitigationLog> {
    ...
    const [newLog] = await db.insert(mitigationLogs).values(log).returning();
    return newLog;
  }

  async getMitigationLogs(limit: number = 20): Promise<MitigationLog[]> {
    if (!db) return [];
    return db.select().from(mitigationLogs)
      .orderBy(desc(mitigationLogs.executedAt))
      .limit(limit);
  }
}
