...
if df.isnull().values.any():
    print("⚠️ Missing values found — applying forward fill.")
...
print("📏 Normalized all numeric columns.")
...
# 2️⃣ Synthetic labels (for demo: 0/1/2/3)
...
for i in range(len(df_scaled) - window_size):
    X.append(df_scaled.iloc[i:i+window_size].values.flatten())
...
pd.DataFrame(X).to_csv(processed_csv_path, index=False)
np.save(X_path, X)
np.save(y_path, y)
